...
#[derive(Subcommand)]
pub enum TimeCmd {
    /// Current time as unix seconds/ms, ISO 8601 (UTC), RFC 2822 and local time (JSON)
    Now,
    ...
    /// Validate a crontab expression and list its next run times (UTC)
    Cron {
...
pub fn run(cmd: TimeCmd) -> Result<()> {
    match cmd {
        TimeCmd::Now => print_json(&report(Utc::now())),
        TimeCmd::Convert { input } => {
